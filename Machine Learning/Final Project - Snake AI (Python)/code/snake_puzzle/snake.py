...
def check_self(xs,ys):
	...
	## check obstacles
	for i in range(len(xs_obst)):
		if collide(xs[0], xs_obst[i], ys[0], ys_obst[i], obj_size, obj_size, obj_size, obj_size):
			return True;
...
def run():
	...
	while True:
		...
		if not DEBUG:
			...
			for i in range(len(xs_obst)):
				s.blit(img_obst, (xs_obst[i], ys_obst[i]))
...
xs_obst = np.array([])
ys_obst = np.array([])
xs_obst_post = np.array([60, 120, 180, 240])

for i in range(20):
	for j in range(len(xs_obst_post)):
		xs_obst = np.append(xs_obst, xs_obst_post[j])
		ys_obst = np.append(ys_obst, 50 + i*obj_size)
...
for i in range(len(xs_obst)):
	grid_orig[xs_obst[i]/obj_size][ys_obst[i]/obj_size] = 3;
...
if not DEBUG:
	...
	img_obst = pygame.Surface((obj_size, obj_size));
	img_obst.fill((255, 185, 20));
